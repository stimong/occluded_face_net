...
def predict_web_cam(model, class_int2char):
    """
    Predict Non-occluded_face from webcamera with tensorflow h5 format model
    Args:
...
def predict_web_cam_tflite(tf_list, class_int2char):
    """
    Predict Non-occluded_face from webcamera with tensorflow tflitte format model
    Args:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--root', "-r",required=False, 
                                   default="./", 
                                    help="프로젝트 경로")
    parser.add_argument('--name', "-n",required=False, default='mobilenet_best', help="불러올 학습데이터 폴더명(날짜)")
